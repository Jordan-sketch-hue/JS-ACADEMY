...
type DemoPatient = { id: string; name: string; email?: string; phone?: string; date_of_birth?: string; gender?: string; address?: string; insurance_plan?: string; risk?: string; balance?: number; ltv?: number; tags?: string[]; };
...
const patientMeds: Record<string, string> = {
  "P-001": "Atorvastatin 20mg, Lisinopril 10mg",
  "P-004": "Metformin 500mg, Aspirin 81mg",
};
...
function ClinicalInner() {
  ...
  useEffect(() => {
    ...
    supabase.from("crown_patients").select("*").order("name").then(({ data }) => {
      if (data && data.length > 0) {
        setPatients(data as DemoPatient[]);
        const pid = params.get("pid") ?? data[0].id;
        setPatientId(pid);
      }
    });
  }, []);

  // Load saved chart + note when patient changes
  useEffect(() => {
    if (!supabase || !patientId) return;
    supabase.from("crown_chart_records").select("tooth_statuses").eq("patient_id", patientId).maybeSingle()
      .then(({ data }) => setToothStatuses((data?.tooth_statuses as Record<string, string>) ?? {}));
    supabase.from("crown_clinical_notes").select("note").eq("patient_id", patientId).maybeSingle()
      .then(({ data }) => setClinicalNote(data?.note ?? ""));
  }, [patientId]);

  async function saveChart() {
    if (!supabase || !patientId) return;
    await supabase.from("crown_chart_records").upsert({ patient_id: patientId, tooth_statuses: toothStatuses, updated_at: new Date().toISOString() });
    flash("Chart saved successfully.");
  }

  async function saveNote() {
    if (!supabase || !patientId) return;
    await supabase.from("crown_clinical_notes").upsert({ patient_id: patientId, note: clinicalNote, updated_at: new Date().toISOString() });
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5 min-w-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <select
              value={patientId}
              onChange={e => { setPatientId(e.target.value); setTranscript(""); }}
          ...
          <div className="flex gap-2">
            ...
            <button onClick={saveChart} className="gold-btn flex items-center gap-2 rounded-lg px-4 py-2 text-sm">
          ...
          <div className="space-y-3">
            {[teeth.slice(0, 16), teeth.slice(16)].map((row, ri) => (
              <div key={ri} className={`flex gap-1 ${ri === 1 ? "flex-row-reverse" : ""}`}>
                {row.map((t) => {
                  const status = toothStatuses[t] ?? "healthy";
                  return (
                    <div key={t} onClick={() => cycleToothStatus(t)} className="flex flex-1 flex-col items-center gap-0.5 cursor-pointer group">
                      <div className={`w-full aspect-square rounded border text-[9px] flex items-center justify-center font-medium transition-all group-hover:border-gold/60 group-hover:shadow-card ${statusColor[status] ?? statusColor.healthy}`}>
                        {status === "missing" ? "—" : t}
                      </div>
        ...
        <div data-guide="clinical-note" className="card bg-white p-5 shadow-card">
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={() => {
              const findings = Object.entries(toothStatuses).filter(([,v]) => v !== "healthy").map(([t,v]) => `Tooth ${t}: ${v}`).join("; ") || "No notable findings";
              setClinicalNote(pt ? `S: ${pt.name} presents for dental examination. ${pt.insurance_plan ? `Insurance: ${pt.insurance_plan}.` : ""}\n\nO: Clinical exam performed. Odontogram findings: ${findings}.\n\nA: Treatment as planned.\n\nP: Procedure completed today. Post-op instructions provided verbally and in writing. Patient tolerated procedure well. Follow-up in 4 weeks.` : "");
            ...
            <button onClick={saveNote} className="gold-btn rounded-lg px-3 py-1.5 text-xs">Accept note</button>
          </div>
        </div>

        {guideActive && <div className="card bg-white p-5 shadow-card">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs min-w-[600px]">
              ...
              <tbody>
                {["B","L"].map((label, ri) => (
                  <tr key={label}>
                    <td className="text-mist pr-3 text-[10px]">{label}</td>
                    {perioData.map((d, i) => (
                      <td key={i} className="text-center pb-0.5">
                        <div className="flex justify-center gap-px">
                          {(ri === 1 ? [...d].reverse() : d).map((v, j) => (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="card bg-white p-5 shadow-card">
            ...
            <p className="text-sm text-slate">{meds ?? "None on file"}</p>
          </div>
          <div className="card bg-white p-5 shadow-card">
            {allergy ? (
      ...
      {showCoach && pt && (() => {
        const acceptScore = pt.risk === "low" ? 85 : pt.risk === "moderate" ? 65 : 45;
        const totalFee = pt.ltv ?? 0;
        const insCover = Math.round(totalFee * 0.6);
        const ptOwes = pt.balance ?? 0;
        const script = `${pt.name.split(" ")[0]}, based on your history and today's exam, we recommend we move forward with treatment. We'll handle the insurance portion directly — your estimated out-of-pocket is $${ptOwes}. We have flexible payment plans if that helps.`;
        return (
            ...
            <div className="p-5 space-y-5 flex-1">
                ...
                <div className="relative inline-flex items-center justify-center">
                  <svg viewBox="0 0 72 72" className="w-20 h-20 -rotate-90">
                    ...
                    <circle cx="36" cy="36" r="30" fill="none" stroke="#C9A96E" strokeWidth="7"
                      strokeDasharray={`${(acceptScore / 100) * 188.5} 188.5`} strokeLinecap="round"/>
                  </svg>
                  <span className="absolute text-xl font-bold text-gold-deep">{acceptScore}%</span>
                </div>
                <p className="text-xs text-slate mt-2">Based on {pt.name.split(" ")[0]}&apos;s history and risk profile</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-ink mb-2">Fee breakdown</p>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-slate">Total fee</span><span className="font-semibold text-ink">${totalFee.toLocaleString()}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-slate">Insurance covers</span><span className="font-semibold text-teal">−${insCover.toLocaleString()}</span></div>
                  <div className="h-px bg-line" />
                  <div className="flex justify-between text-sm"><span className="font-semibold text-ink">Patient owes</span><span className="font-bold text-ink">${ptOwes}</span></div>
                </div>
              </div>
              {ptOwes > 200 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {[{ plan: "3 months", payment: Math.round(ptOwes / 3) }, { plan: "6 months", payment: Math.round(ptOwes / 6) }, { plan: "CareCredit 12mo", payment: Math.round(ptOwes / 12) }].map((f) => (
                      <div key={f.plan} className="flex items-center justify-between rounded-lg border border-line bg-surface px-3 py-2.5">
                        <div><p className="text-xs font-semibold text-ink">{f.plan}</p><p className="text-[10px] text-mist">0% interest</p></div>
              ...
              <div>
                ...
                <div className="rounded-xl border border-gold/30 bg-gold/5 p-3">
                  <p className="text-xs text-ink leading-relaxed italic">&ldquo;{script}&rdquo;</p>
                </div>
                <button onClick={() => { navigator.clipboard?.writeText(script).then(() => flash("Script copied to clipboard.")); }} className="mt-2 w-full rounded-lg border border-line py-2 text-xs text-slate hover:bg-surface hover:border-gold/40 transition-colors flex items-center justify-center gap-1.5">
