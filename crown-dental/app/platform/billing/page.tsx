...
const invoices: { id: string; patient: string; proc: string; amount: string; ins: string; pt: string; date: string; status: string }[] = [];
...
export default function BillingPage() {
  ...
  const [inv, setInv] = useState({ patient: "", procedure: "", amount: "", insurance: "", patient_portion: "", date: new Date().toISOString().slice(0,10) });
  ...
  function addInvoice() {
    ...
    setInvoiceList(prev => [{
      id, patient: inv.patient, proc: inv.procedure,
      amount: `$${inv.amount}`, ins: `$${inv.insurance || "0"}`,
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => {
            const rows = ["Invoice ID,Patient,Procedure,Amount,Insurance,Patient Portion,Date,Status", ...invoiceList.map(inv => `${inv.id},${inv.patient},"${inv.proc}",${inv.amount},${inv.ins},${inv.pt},${inv.date},${inv.status}`)];
            const blob = new Blob([rows.join("\n")], { type: "text/csv" });
            const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "crown-billing.csv"; a.click(); URL.revokeObjectURL(url);
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <select value={inv.patient} onChange={e => setInv(f => ({...f, patient: e.target.value}))}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate mb-1.5">Procedure <span className="text-alert">*</span></label>
                <input type="text" value={inv.procedure} onChange={e => setInv(f => ({...f, procedure: e.target.value}))}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="number" value={inv.amount} onChange={e => setInv(f => ({...f, amount: e.target.value}))}
                    placeholder="0"
                    className="w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink focus:border-gold/60 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate mb-1.5">Insurance ($)</label>
                  <input type="number" value={inv.insurance} onChange={e => setInv(f => ({...f, insurance: e.target.value}))}
                    placeholder="0"
                    className="w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink focus:border-gold/60 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate mb-1.5">Patient ($)</label>
                  <input type="number" value={inv.patient_portion} onChange={e => setInv(f => ({...f, patient_portion: e.target.value}))}
                    placeholder="0"
                    className="w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink focus:border-gold/60 focus:outline-none" />
                </div>
              </div>
              <div>
                ...
                <input type="date" value={inv.date} onChange={e => setInv(f => ({...f, date: e.target.value}))}
                  className="w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ink focus:border-gold/60 focus:outline-none" />
              </div>
            </div>
            <div className="flex gap-3 border-t border-line px-5 py-4">
              ...
              <button onClick={addInvoice} disabled={!inv.patient || !inv.procedure || !inv.amount} className="flex-1 gold-btn rounded-xl py-2.5 text-sm disabled:opacity-40">Create Invoice</button>
            </div>
          </div>
        </div>
      )}
          ...
          return (
            <div key={s.l} className="card bg-white rounded-xl p-4 shadow-card">
              <div className="flex items-center gap-3 mb-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-surface"><I className="h-4 w-4 text-gold-deep" /></div>
                {s.delta && <span className="ml-auto rounded-full bg-teal/10 px-2 py-0.5 text-[11px] font-semibold text-teal">{s.delta}</span>}
              </div>
      ...
      {invoiceList.filter(i => i.status === "Overdue").length > 0 && (
        <div className="flex items-start gap-3 rounded-xl border border-alert/30 bg-alert/5 p-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-ink">{invoiceList.filter(i => i.status === "Overdue").length} overdue invoice(s)</p>
            <p className="text-xs text-slate mt-0.5">Crown AI can send automated payment reminders for overdue balances.</p>
          </div>
          <button onClick={() => { setReminded(true); }} disabled={reminded} className="shrink-0 gold-btn rounded-lg px-3 py-1.5 text-xs disabled:opacity-60">
            ...
            {invoiceList.length === 0 ? (
              ...
              invoiceList.map((inv) => (
                <tr key={inv.id} className="hover:bg-surface transition-colors">
                  <td className="px-5 py-3 font-mono text-xs text-mist">{inv.id}</td>
                  <td className="px-5 py-3 font-medium text-ink">{inv.patient}</td>
                  <td className="px-5 py-3 text-slate">{inv.proc}</td>
                  <td className="px-5 py-3 font-semibold text-ink">{inv.amount}</td>
                  <td className="px-5 py-3 text-slate">{inv.ins}</td>
                  <td className="px-5 py-3 text-slate">{inv.pt}</td>
                  <td className="px-5 py-3 text-mist">{inv.date}</td>
                  <td className="px-5 py-3">
                    <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${statusStyle[inv.status]}`}>{inv.status}</span>
                  </td>
                  <td className="px-5 py-3">
                    <Link href={`/platform/patients/${patientIdMap[inv.patient] ?? "search"}`} className="rounded-lg border border-line px-3 py-1 text-xs text-ink hover:bg-surface hover:border-gold/40 transition-colors">View</Link>
                  </td>
                </tr>
              ))
            )}
      ...
      <div className="card bg-white p-5 shadow-card">
        <div className="flex items-center gap-3 mb-4">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gold/10"><Icons.Star className="h-5 w-5 text-gold-deep" /></div>
          <div>
